...
river_zip = ("../data/records-2318770.zip")
lake_zip = ("../data/records-2309514.zip")
try(importData(type = 'zip', filepath = c(river_zip, lake_zip)))
try(importData(type = 'zip', filepath = c("records-2318770.zip", "records-2309514.zip")))
```

You can download GLKN water packages from DataStore using the `NPSutils` package, or you can go to DataStore on IRMA and download the data packages. Code to download via `NPSutils` is below. Once the data packages are downloaded on your computer, you can run the `importData()` function like above.

```{r eval = F, include = T}
# Download lakes and rivers data package from DataStore
pak::pkg_install("doi-nps/NPSutils") # might take a few tries to install
NPSutils::get_data_packages(c("2318770", "2309514"))
```
...
```{r, eval = F, include = T}
importData(type = 'csv', filepath = "./data/2318770") # update filepath to match your computer
```
...
```{r eval = F, include = T}
importData(type = 'csv', filepath = "./data/2309514") # update filepath to match your computer
```
...
By importing both simultaneously, each of the views (e.g. Locations, Results, etc.) are row bound to include both rivers and lakes data in one view. Note that if you import both the rivers and lakes data in the same function call, columns not in common between the two datasets will not be included (e.g., Primary_Aquifer and Local_Aquifer in Rivers, Aquifer_Name in Lake DP).
```{r eval = F, include = T}
river_zip = ("../data/records-2318770.zip")
lake_zip = ("../data/records-2309514.zip")
...
<li>View code in the <a href="https://github.com/doi-nps/waterGLKN/tree/main">GitHub doi-nps/waterGLKN repo</a>. The functions are in the R folder. 
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(isro7)}
isro7 <- plotLakeProfile(site = "ISRO_07", parameter = "TempWater_C", years = 2007:2023)
isro7
```

<p class = 'fxn'>Plot water temperature for all PIRO lakes sampled in 2023, with fixed Y axis range.</p>
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(piroC)}
piroC <- plotLakeProfile(park = "PIRO", parameter = "TempWater_C", years = 2023, palette = "Spectral")
piroC
```

<p class = 'fxn'>Plot water temperature for all PIRO lakes sampled in 2023, with varying Y axis range.</p>
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(piroCfy)}
piroCfy <- 
...
```{r message = FALSE, warning = F,fig.alt = get_alt_text(voya1)}
voya1 <- plotLakeProfile(site = "VOYA_01", parameter = "TempWater_C")
voya1
```

<p class = 'fxn'>Plot Specific Conductance for SLBE_01 all years without theromocline or plot title</p>
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(slbe1)}
slbe1 <- plotLakeProfile(site = "SLBE_01", parameter = "SpecCond_uScm", plot_thermocline = F, plot_title = F)
slbe1
```
<p class = 'fxn'>Plot water temp using Red-Yellow-Blue palette </p>
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(piro1C)}
piro1C <- 
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(pirodo)}
pirodo <- 
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(sacn_ph)}
lkst <- c("SACN_STCR_20.0", "SACN_STCR_15.8", "SACN_STCR_2.0")
sacn_ph <- plotLakeProfile(site = lkst, parameter = "pH", years = 2023)
sacn_ph
```
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(voya_scat)}
voya_scat <- 
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(voya_scat2)}
voya_scat2 <- 
plotScatterPlot(park = "VOYA", parameters = c("DO_mgL", "TempWater_C"),
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(piro_scat4)}
piro_scat4 <- 
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(isro_chla)}
isro_chla <- plotTrend(site = "ISRO_01", parameter = "ChlA_ugL")
isro_chla
```

<p class = 'fxn'>Same as above, but change palette, add x gridlines and include all sample depths, and censored points. </p>
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(isro_chla2)}
isro_chla2 <- 
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(voya1_ph)}
voya1_ph <- plotTrend(site = "VOYA_01", parameter = "pH", palette = 'dimgrey', years = 2013:2023)
voya1_ph
```
<p class = 'fxn'>Plot smoothed surface pH for PIRO_02 for all years, with turbo palette, and using span of 0.75. </p>
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(piro2_ph)}
piro2_ph <- 
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(voya17_so4)}
voya17_so4 <-
...
<p class = 'fxn'>Plot smoothed surface pH for 4 lakes in ISRO over all years with 0.6 span.</p>
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(isro4_ph)}
isro4_ph <- 
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(piro_3p)}
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(isro_4p)}
isro_4p <-
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(sacn_bands)}
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(sacn_chla)}
sacn_chla <- 
plotWaterBands(site = lksc, year_curr = 2023, years_historic = 2007:2022, 
...
```{r message = FALSE, warning = FALSE, fig.alt = get_alt_text(isro_bands)}
